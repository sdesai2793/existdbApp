...
const _inverseTransition = transition => {
  ...
  for (const [key, value] of transition.mapping) {
    ...
    if (value.ref) {
      ...
      const existingMapping = inverseTransition.mapping.get(value.ref[0])
      if (!existingMapping) inverseTransition.mapping.set(value.ref[0], mapped)
      else {
        const alternatives = existingMapping.alternatives || []
        alternatives.push(mapped)
        existingMapping.alternatives = alternatives
        inverseTransition.mapping.set(value.ref[0], existingMapping)
      }
    }
  }
  ...
  for (const key in newData) {
    ...
    if (mapped && mapped.ref) {
      ...
      if (mapped.alternatives)
        mapped.alternatives.forEach(({ ref }) => {
          newData[ref[0]] = value
        })
    }
  }
...
const _renameColumns = (target, columns, force = false) => {
  ...
  const renamedColumns = columns.map(column => {
    ...
    if (queryColumn) {
      ...
      if (queryColumn.ref) renamedColumn.ref = [queryColumn.ref[queryColumn.ref.length - 1]]
...
const _filterColumnsOnce = (target, columns = [], force) => {
  ...
  const plainColumns = cqnColumns
    ...
    .filter(c => c !== '*' && !(c.ref && c.ref.some(ele => typeof ele !== 'string')))
    .map(c => ({ ref: (c.as && [c.as]) || c.ref }))
  _includeForeignKeys(columns, target, force)
  const query = target.query

  if (query && query._target && cqnColumns.some(c => c === '*')) {
    // 'alias' is required to properly perform mapping
    const starColumns = _starColumns(query._target.elements).map(c => (c.as && c) || { as: c.ref[0], ref: c.ref })
