...
exports.checkMandatoryValue = exports.readPropertyWithWarn = exports.wrapJwtInHeader = exports.audiences = exports.issuerUrl = exports.verifyJwtWithKey = exports.verificationKeyCache = exports.verifyJwt = exports.retrieveJwt = exports.decodeJwtComplete = exports.decodeJwt = void 0;
...
function checkDomainVerificationKeyURL(verificationKeyURL, uaaDomain) {
    ...
    if (!uaaDomain || !jkuDomain || !jkuDomain.endsWith(uaaDomain)) {
        throw new Error("The domains of the XSUAA and verification URL do not match. The XSUUA domain is '" + uaaDomain + "' and the jku field provided in the JWT is '" + jkuDomain + "'.");
    }
}
/**
...
function verifyJwtWithKey(token, key) {
    return new Promise(function (resolve, reject) {
        jsonwebtoken_1.verify(token, sanitizeVerificationKey(key), function (err, decodedToken) {
            if (err) {
                reject(new util_1.ErrorWithCause('Invalid JWT.', err));
            }
...
function audiencesFromScope(decodedToken) {
    ...
    return scopes.reduce(function (aud, scope) {
        if (scope.includes('.')) {
            return __spreadArray(__spreadArray([], aud), [scope.substr(0, scope.indexOf('.'))]);
        }
        return aud;
    }, []);
}
