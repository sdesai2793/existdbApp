...
function filterSelects(selects) {
    ...
    return __spreadArray(__spreadArray([], allFieldSelects), selects.filter(function (select) { return !selectionLevels.includes(selectionLevel(select)); }));
}
function getSelectsAsStrings(selectables, initialSelect, parent) {
    ...
    return selectables.reduce(function (select, selectable) {
        ...
        if (selectable instanceof odata_common_1.Link) {
            ...
            return __spreadArray(__spreadArray([], select), [fullFieldName + "/*"]);
        }
        return __spreadArray(__spreadArray([], select), [fullFieldName]);
    }, initialSelect);
}
